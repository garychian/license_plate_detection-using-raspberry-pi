...
class YoloLayer(Layer):
    def __init__(self, anchors, max_grid, batch_size, warmup_batches, ignore_thresh,
                 ...
                 **kwargs):
        # make the model setting persistent
        ...
        max_grid, max_grid_w = max_grid

        cell_x = tf.to_float(tf.tile(tf.range(max_grid_w), [max_grid_h]), (1, max_grid_h, max_grid_w, 1, 1)))
    ...
    def call(self,x):
        input_image, y_true, true_boxes = x

        # adjust the shape of the y_predit [batch, grid_h, grid_w, 3, 4+11+nb_class]
        y_pred = tf.reshpe(y_pred, tf.concat([tf.shape(y_pred)[:3], tf.constant([3,-1])], axis = 0))
        ...
        pred_xy = tf.expend_dims(pred_box_xy / grid_factor,4)
        pred_wh = tf.expend_dims(tf.exp(pred_box_wh) * self.anchors / net_factor,4)
        ...
        conf_delta *= tf.expend_dims(tf.to_float(best_ious < self.ignore_thresh), 4)
